...
local console = {
  font = "enter_command",               -- Font of the console (monospaced).
  font_color = {224, 255, 208},         -- Font color.
  font_size = 16,                       -- Font size in pixels.
  font_height_in_pixel = 10,
  char_width = 6,                       -- Character width in pixels.
  line_spacing = 3,                     -- Space between two lines in pixels.

  margin = 8,                           -- Margin of the console in pixels.
  padding = 5,                          -- Padding of the console in pixels.

  color = {15, 31, 32},                -- Background color of the console.
  opacity = 255,                        -- Background opacity of the console.
  selection_color = {48, 111, 80},    -- Color of the selection.
  selection_opacity = 128,              -- Selection opacity.
...
function console:init()
  ...
  self.line_height = self.font_height_in_pixel + self.line_spacing
  self.max_lines = math.floor(inner_height / self.line_height)

  local spacing = inner_height - (self.font_height_in_pixel * self.max_lines)
  self.line_height = self.font_height_in_pixel + math.floor(spacing / self.max_lines)
...
function console:get_cursor_position(cursor)
  ...
  local text = "> " .. table.concat(self:get_current_line())
  local str_len = string.len(text)
  local text2 = console.text_surfaces[console.current_line]:get_text()
  local size2 = self.text_surfaces[console.current_line]:get_size()
  local str_len2 = string.len(text2) - 1
  print(text, str_len, text2, str_len2, size2)
...
function console:on_draw(dst_surface)
  ...
  local x = origin
  local y = x
  --local n = #self.text_surfaces
  --print("Nombre de lignes: "..n)
  -- local line = "> " .. table.concat(self:get_current_line())
  -- local position = string.len(line) % 31
  -- print(line, position)
  --print(console.current_line)
  -- local index = math.min(self.current_line - 1, self.max_lines)
  -- print(32 % 30)
  -- console.print("TOTO")
  --print(self.text_surfaces[console.current_line]:get_text())
  --print(self.text_surfaces[console.current_line]:get_size())
  --if self.current_line == i then
    --print(i, i, i, i, i, i, i ,i ,i ,i, i, i)
    --local txt = self.text_surfaces[i]:get_text()
    --if not (txt == "") then
      --print(txt)
      --local w, h = self.text_surfaces[i]:get_size()
